...
import {
  Users, Search, UserPlus, X, Mail, ArrowRight,
} from "lucide-react";
...
export default function PatientsPage() {
  ...
  const [lookupResult, setLookupResult] = useState<{ id: string; full_name: string; email: string } | null>(null);
  const [lookupError, setLookupError] = useState("");
  const [assigning, setAssigning] = useState(false);
  const [lookupLoading, setLookupLoading] = useState(false);

  async function loadAssignments() {
    const res = await fetch("/api/rd/patients");
    if (res.ok) {
  ...
  const filtered = assignments.filter((a) => {
    ...
    return a.profiles.fullName?.toLowerCase().includes(q) || a.profiles.email?.toLowerCase().includes(q);
  });
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 flex items-center gap-2">
            <Users className="w-6 h-6 text-accent-600" /> Patients
          </h1>
          <p className="text-gray-500 mt-1">Manage your patient roster and assignments.</p>
        </div>
        ...
        ><UserPlus className="w-4 h-4" /> Add Patient</button>
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-12 bg-white rounded-xl border border-gray-200">
          ...
          <p className="text-sm text-gray-400 mt-1">Use &quot;Add Patient&quot; to assign patients to your roster.</p>
        </div>
      ) : (
        <div className="bg-white rounded-xl border border-gray-200 divide-y divide-gray-100">
          {filtered.map((a) => (
            <Link key={a.id} href={`/rd/patients/${a.patientId}`}
            ...
            >
              <div className="flex items-center gap-4">
                <div className="w-10 h-10 bg-accent-100 rounded-full flex items-center justify-center text-accent-600 font-semibold text-sm">
                  {a.profiles.fullName ? a.profiles.fullName.split(" ").map((n) => n[0]).join("").toUpperCase().slice(0, 2) : "??"}
                </div>
                <div>
                  <p className="font-medium text-gray-900">{a.profiles.fullName || "Unknown Patient"}</p>
                  <p className="text-sm text-gray-500">{a.profiles.email || "No email"}</p>
                </div>
              </div>
              <div className="flex items-center gap-3">
                <span className={`text-xs font-medium px-2.5 py-0.5 rounded-full ${statusColors[a.status] || statusColors.active}`}>{a.status}</span>
        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-xl max-w-md w-full p-6">
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => { setShowAddModal(false); setLookupEmail(""); setLookupResult(null); setLookupError(""); }}>
                <X className="w-5 h-5 text-gray-400 hover:text-gray-600" />
              </button>
            </div>
            <p className="text-sm text-gray-500 mb-4">Search for a patient by their email address to add them to your roster.</p>
            ...
            {lookupError && <div className="bg-red-50 text-red-600 text-sm rounded-lg p-3 mb-4">{lookupError}</div>}
            {lookupResult && (
              <div className="bg-gray-50 rounded-lg p-4 mb-4">
                <p className="font-medium text-gray-900">{lookupResult.full_name}</p>
                ...
                >{assigning ? "Assigning..." : "Assign to My Roster"}</button>
              </div>
            )}
          </div>
        </div>
