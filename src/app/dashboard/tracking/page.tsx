...
import {
  Weight, Activity, Plus, TrendingUp, Calendar, Save,
} from "lucide-react";

const symptomOptions = [
  "Nausea", "Vomiting", "Diarrhea", "Constipation", "Bloating",
  "Abdominal pain", "Reflux", "Gas", "No symptoms",
];
...
export default function TrackingPage() {
  ...
  async function handleSave() {
    ...
    if (res.ok) {
      ...
      if (log) setRecentLogs((prev) => [log, ...prev]);
    }
    ...
    <div className="max-w-4xl mx-auto space-y-8">
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-6 space-y-5">
            ...
            {symptomOptions.map((s) => (
              <button key={s} onClick={() => toggleSymptom(s)}
                className={`text-sm px-3 py-1.5 rounded-lg border transition ${
                  symptoms.includes(s) ? "bg-brand-50 border-brand-300 text-brand-700" : "border-gray-200 text-gray-600 hover:border-gray-300"
                }`}
              >{s}</button>
            ))}
          </div>
          ...
          <div>
            ...
            <div className="flex gap-3">
              {severityLevels.map((level) => (
                <button key={level.value} onClick={() => setSeverity(level.value)}
                  className={`px-4 py-2 rounded-lg text-sm font-medium border-2 transition ${
                    severity === level.value ? `${level.color} border-current` : "border-gray-200 text-gray-500 hover:border-gray-300"
                  }`}
                >{level.label}</button>
              ))}
            </div>
        ...
        <label className="flex items-center gap-3 cursor-pointer">
          <input type="checkbox" checked={intakeCompleted} onChange={(e) => setIntakeCompleted(e.target.checked)}
        ...
        ><Save className="w-4 h-4" /> {saving ? "Saving..." : "Save Log Entry"}</button>
      </div>
                  ...
                  <div className="flex items-center gap-3 text-sm">
                    {log.weight && (
                      <span className="text-gray-600">
                        <Weight className="w-4 h-4 inline mr-1" />{log.weight} lbs
                      </span>
                    )}
