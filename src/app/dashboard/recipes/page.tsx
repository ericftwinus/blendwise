...
export default function RecipesPage() {
  ...
  async function saveRecipe(recipe: Recipe) {
    const res = await fetch("/api/dashboard/recipes", {
      ...
      body: JSON.stringify({
        ...
        volumeMl: recipe.volume_ml || parseInt(recipe.volume?.replace(/\D/g, "") || "0"),
    ...
    if (res.ok) {
      ...
      setSavedRecipes((prev) => [{ ...recipe, id: saved.id, saved: true }, ...prev]);
    }
  }
  ...
  const filtered = displayRecipes.filter((r) => {
    const ingredientNames = r.ingredients.map((i) => typeof i === "string" ? i : i.name);
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8">
      ...
      <div className="flex gap-2">
        <button onClick={() => { setShowSaved(false); setSelectedTag(""); }}
          className={`px-4 py-2 rounded-lg text-sm font-medium transition ${!showSaved ? "bg-brand-600 text-white" : "bg-gray-100 text-gray-600 hover:bg-gray-200"}`}
        >Generated ({recipes.length})</button>
        <button onClick={() => { setShowSaved(true); setSelectedTag(""); }}
          className={`px-4 py-2 rounded-lg text-sm font-medium transition flex items-center gap-1.5 ${showSaved ? "bg-brand-600 text-white" : "bg-gray-100 text-gray-600 hover:bg-gray-200"}`}
        ><Bookmark className="w-4 h-4" /> Saved ({savedRecipes.length})</button>
      </div>

      <div className="flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1">
          <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-400" />
          <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search recipes or ingredients..."
        ...
        {allTags.length > 0 && (
          <div className="flex items-center gap-2 overflow-x-auto pb-1">
            ...
            <button onClick={() => setSelectedTag("")}
              className={`px-3 py-1.5 rounded-lg text-xs font-medium whitespace-nowrap transition ${!selectedTag ? "bg-brand-600 text-white" : "bg-gray-100 text-gray-600 hover:bg-gray-200"}`}
            >All</button>
            {allTags.map((tag) => (
              <button key={tag} onClick={() => setSelectedTag(tag === selectedTag ? "" : tag)}
                className={`px-3 py-1.5 rounded-lg text-xs font-medium whitespace-nowrap transition ${tag === selectedTag ? "bg-brand-600 text-white" : "bg-gray-100 text-gray-600 hover:bg-gray-200"}`}
              >{tag}</button>
            ))}
          </div>
        )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filtered.map((recipe, idx) => {
          const ingredientNames = recipe.ingredients.map((i) => typeof i === "string" ? i : `${i.amount} ${i.name}`);
            ...
            <div key={recipe.id || idx} className="bg-white rounded-xl border border-gray-200 overflow-hidden hover:shadow-md transition group">
              ...
              <div className="p-5">
                <div className="flex items-center gap-2 mb-2 flex-wrap">
                  {recipe.tags.map((tag) => (
                    <span key={tag} className="text-xs bg-brand-50 text-brand-600 px-2 py-0.5 rounded-full font-medium">{tag}</span>
                  ))}
                </div>
                <h3 className="text-lg font-semibold text-gray-900 mb-1 group-hover:text-brand-600 transition">{recipe.name}</h3>
                ...
                <div className="flex items-center gap-4 text-xs text-gray-500 mb-3">
                  ...
                  <span className="flex items-center gap-1"><strong>P:</strong> {recipe.protein}g</span>
                  {volumeStr && <span className="flex items-center gap-1"><Droplets className="w-3.5 h-3.5" /> {volumeStr}</span>}
                  {(recipe.prep_time || recipe.prepTime) && (
                    <span className="flex items-center gap-1"><Clock className="w-3.5 h-3.5" /> {recipe.prep_time || recipe.prepTime}</span>
                  )}
                </div>
                <div className="mb-3">
                ...
                {recipe.instructions && (
                  <details className="text-xs text-gray-600 mb-3">
                    <summary className="cursor-pointer font-medium text-brand-600 hover:underline">View Instructions</summary>
                    <p className="mt-2 whitespace-pre-line bg-gray-50 rounded-lg p-3">{recipe.instructions}</p>
                  </details>
                )}
                {!showSaved && (
                  <button onClick={() => saveRecipe(recipe)} className="flex items-center gap-1.5 text-xs text-brand-600 font-medium hover:underline">
      ...
      {filtered.length === 0 && (
        <div className="text-center py-12 text-gray-500">
          ...
          <p className="text-sm">{showSaved ? "Generate recipes and save your favorites." : "Try adjusting your filters or generate new recipes."}</p>
        </div>
      )}
    </div>
  );
