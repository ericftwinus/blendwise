...
export default function PatientMessagesPage() {
  ...
  const remoteName = remoteUserId
    ? selectedConv?.participantNames?.[remoteUserId] || "Your RD"
    : "Your RD";

  function getOtherName(conv: Conversation) {
    const otherId = conv.participants.find((p) => p !== user!.uid);
    return otherId ? conv.participantNames?.[otherId] || "Your RD" : "Your RD";
  }
    ...
    return (
      <div className="text-center py-16">
        ...
        <p className="text-gray-500 text-sm">
          Your RD will start a conversation when they review your profile.
        </p>
      </div>
    );
  }
          ...
          <>
            <div className="p-3 border-b border-gray-100 flex items-center justify-between">
              ...
              {remoteUserId && (
                <button
                  onClick={() => setShowVideo(true)}
                  className="flex items-center gap-1.5 text-xs px-3 py-1.5 bg-brand-600 text-white rounded-lg font-medium hover:bg-brand-700 transition"
                >
